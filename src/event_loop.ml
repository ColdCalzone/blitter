...
let list_replace list i value =
  List.mapi list ~f:(fun idx orig -> match i = idx with true -> value | false -> orig)
;;
  ...
  let new_timer =
    ...
    | Timing (splits, start_time) ->
      ...
      | "start-split-reset" | "start-split" ->
        ...
        let new_state =
          if List.length new_splits = List.length timer.split_names
          then Timer_state.Done (new_splits, start_time)
