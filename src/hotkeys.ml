...
let stream_of_python python_src =
  ...
  let%lwt n =
    Lwt_unix.write_string pipe_out_fd_unix python_src 0 (String.length python_src)
  in
  if n < String.length python_src
  then failwith "Failed to write python to pipe"
  else
